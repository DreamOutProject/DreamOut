...
public class Main {
    ...
    private static RoomPanel present=null;
    public static void Transition_go(){
        frame.getContentPane().removeAll();
        frame.add(present);
        frame.revalidate();
        frame.repaint();
    }
    public synchronized static void setPanel(RoomPanel t){
        present = t;
    }
    ...
    public static void test(){
        try {
            ...
            reapaintThread tp = new reapaintThread();
            tp.start();
            tp.join();
        ...
        public void run() {
            ...
            while(true){
                try{
                    ...
                    if(present instanceof GameStartRoom)continue;//게임 중이면 리페인트 안 하기
                    if(msg.getMsgMode() == ObjectMsg.REPAINT_MODE){//화면 다시 그려주기
                        if(present instanceof WaitRoom){//기다리는 중이라면 화면을
                            setPanel(new WaitRoom(frame));
                            Transition_go();//다시 화면 돌리기
                        }else if(present instanceof GameRoom){//그림 그려야 되는 경우
                            setPanel(new GameRoom(frame));
                            Transition_go();
                        }
